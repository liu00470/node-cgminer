...
var singular = ["VERSION", "SUMMARY", "CHECK"];
var replyMapping = {
  check: "CHECK",
};
...
function createCommandMethod(command) {
  this[command.name] = function (args) {
    return this.request(command, args).then(
      function (result) {
...
CGMinerClient.prototype.connect = function (data) {
  ...
  if (data == "stats") return this.stats();
  else if (data == "pools")
    return this.pools()
      .then(function (data) {
        console.log("***********************************", data);
...
CGMinerClient.prototype.request = function (command, args) {
  ...
  socket.on("connect", function () {
    ...
    socket.on("end", function () {
      ...
      var str = buffer.replace(/\-nan/g, "0").replace(/[^\x00-\x7F]/g, "");
      try {
        if (/RESTART/.test(str)) {
          json = {
            STATUS: [
        ...
        if (command.reply === null) {
          if (_.similar(templates["null"], json.STATUS[0])) {
            ...
            return deferred.reject({
              msg: "response did not match template",
              response: json,
            });
          }
        }
